...
void __md_error(struct mddev *mddev, struct md_rdev *rdev, bool writeout)
{
	...
	if (!mddev->pers || !mddev->pers->error_handler)
		return;
	mddev->pers->error_handler(mddev, rdev, writeout);
	if (mddev->degraded)
		set_bit(MD_RECOVERY_RECOVER, &mddev->recovery);
	sysfs_notify_dirent_safe(rdev->sysfs_state);
	if (writeout) {
...
static int __remove_and_add_spares(struct mddev *mddev, struct md_rdev *this, bool writeout)
{
	struct md_rdev *rdev;
	int spares = 0;
	int remove_some = 0;
...
static int remove_and_add_spares(struct mddev *mddev,
				 struct md_rdev *this)
{
	return __remove_and_add_spares(mddev, this, true);
}
	...
	rdev_for_each(rdev2, mddev) {
		...
		if (role != rdev2->raid_disk) {
			...
			if (rdev2->raid_disk == -1 && role != 0xffff) {
				int ret;
				__remove_and_add_spares(mddev, rdev2, false);
